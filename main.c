...
void
get_client_address(struct sockaddr_in client_address)
{
...
void
perform_connection(int connection_socket, int listen_socket, struct sockaddr_in client_address, int address_lenght)
{
    char buffer[BUFSIZE];
    int n;
...
void
bind_to_port(int listen_socket, struct sockaddr_in server_address)
{
...
int
calculate(char *input, int *result) 
{
...
void
bin_op_stack(Stack *stack, int (*f)(int, int))
{
    int popped_value1;
...
char
handle_token(Stack *stack, char *token) 
{
    if(isnumber(token) == 1)
